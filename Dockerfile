...
LABEL maintainer="Ihor Furseyev <furseyev@gmail.com>"
LABEL org.label-schema.schema-version="1.0"
LABEL org.label-schema.name="furseyev/magento2"
LABEL org.label-schema.description="Opinionated docker image for Magento 2.2+."
LABEL org.label-schema.vcs-url="https://github.com/furseyev/docker-magento2"
...
RUN apk add --no-cache --virtual .ext-deps $EXTENSION_DEPS \
 && docker-php-ext-install -j$(nproc) bcmath intl opcache pdo_mysql soap xsl zip sockets iconv \
 && docker-php-ext-configure gd --with-freetype-dir=/usr/include/ --with-jpeg-dir=/usr/include/ \
 && docker-php-ext-install -j$(nproc) gd \
 && apk add --no-cache --virtual .build-deps $PHPIZE_DEPS \
 && pecl install xdebug-2.9.2 mcrypt-1.0.3 \
 && apk del .build-deps \
 && apk del .ext-deps \
 && rm -rf /tmp/pear
